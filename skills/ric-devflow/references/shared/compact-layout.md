...
新 Root Issue 的默认存储契约。不是独立 Skill，也不是另一个流程。G0–G10、Verdict、状态和原报告载荷继续使用既有契约。
...
模板：[State](../../assets/templates/compact/state.yaml)、[Current](../../assets/templates/compact/current.md)、[Test Plan](../../assets/templates/compact/test-plan.md)、[Evidence](../../assets/templates/compact/evidence.md)。v2 容器以 `schema_version: 2` 标记；嵌入的原 Review/Test/Implementation/Defect 载荷仍用原模板与 `schema_version: 1`，保留所有字段和未知扩展，不把容器版本写入原报告。
...
```
`path` 为仓库相对文件路径，`id` 定位该文件内对象；`repository.root`、`cwd`、正文、附件和链接统一遵循[持久文件路径](../contracts/artifact-lifecycle.md#持久文件路径)。仓库绝对位置仅供会话与工具调用定位，不写入交接文件或正式记录。文档 `commit_sha` 必须是包含该路径/对象的完整已存在 Commit。使用 `git show <doc_sha>:<path>` 读取受审内容，不能以当前工作树或移动分支替代。局部 Task 审核只读所指章节和相关邻域，不必读取整个账本。
...
命令证据须包含完整 SHA（适用时）、仓库相对 cwd、命令、脱敏环境/依赖版本、复现数据条件、退出码、关键输出、范围、未运行原因。作者按持久文件路径规则记录命令与输出；摘要不足以复现时，把必要原始脱敏材料提升到 `attachments/` 或稳定仓库/CI 产物，不能仅给临时路径。不得把失败或 BLOCKED 报告当垃圾删掉。
