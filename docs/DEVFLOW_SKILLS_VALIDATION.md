...
## 2026-09-14：持久产物相对路径规则

### 受检对象与范围

- 采集时间：`2026-09-14T00:24:23+08:00`；Linux/Bash，Python `3.14.4`、PyYAML `6.0.3`。版本来自本轮实际检查输出。
- 起点 HEAD：`e8977b41f3d6949e75a61cdcefb527dec8302049`；受检对象为其上的未提交工作区，最终候选集合 SHA256：`984db80e1bcfe26ca3d71f961416df3b6f38b170291f5963a088f43a5bb6b1f8`。采用验证指南的文件集算法，报告自身不进入集合摘要；该摘要不是产品 tested SHA。
- 共享生命周期统一新写入文件的相对路径基准；Compact 与交接规则引用同一契约；四个 Compact 模板补充路径说明，state 默认根为 `.`、current 为仓库相对路径。旧 v1 模板不改，历史短引用按原布局读取，已发布载荷与快照保持原文。
- 追加前报告与起点 HEAD 逐字一致：78943 字节，SHA256 `12fb194086e6b4d302ffa995d58ed6eb23cffe5baf00335a3d7f6d804cd7dc09`；本轮只追加此节。

### 实际执行与结果

- 每次 Python 调用均设置 `PYTHONUTF8=1 PYTHONIOENCODING=utf-8`。从验证指南第一个完整块提取 Python 正文，原样交给 `python3 -B -c` 执行；通过当前命令的 `DEVFLOW_VALIDATOR_PATH` 指向已确认的 Codex bundled `skill-creator/scripts/quick_validate.py`，无用户配置修改。
- 最终静态检查退出码 0：五 Skill 验证全部通过；解析 13 YAML、5 TOML、16 frontmatter、86 Markdown；182 当前链接/锚点、3 历史链接通过。四角色引用闭包保持自包含且不依赖无关角色。
- 模板兼容检查通过：16 个 v1 模板与既有完整基线逐字一致；4 个 Compact 模板仅含允许的路径默认值及说明调整，其余载荷保持兼容。
- 隔离文件检查退出码 0：从四个实际 Compact 模板替换 REQ-ID 生成临时目录，解析 state 与 evidence 中 2 个仓库相对引用；将整个 checkout 移到含中文与空格的新目录、确认旧位置消失后，相同引用仍可解析。此项仅证明模板引用可迁移，不是 Agent 行为评测。
- 兼容检查器负控：分别向隔离副本注入 v1 模板变化、Compact state 未知字段、Compact Markdown 正文变化，三个子进程均按预期退出 1，并命中对应兼容断言；干净对照退出 0。临时副本已精确清理。
- `git diff --check` 与静态块中的暂存差异检查通过；已复核本轮源码差异与工作区状态。

### 未运行项与限制

- 未运行 Codex、Claude Code、ZCode 的原生角色调用或端到端生成；不把静态规则检查或目录搬迁检查声称为宿主已遵循指令。
- 未批量修改 `.devflow/changes/` 中的既有报告、快照或演示记录，也未执行旧布局迁移、安装同步、提交或推送。既有未跟踪 `.codex/` 未纳入改动。


## 2026-09-16 — 单功能分支、detached worktree 与渐进加载

本轮执行已批准的源码维护计划：每 Root 默认一个功能分支兼作集成分支，无命名规范时使用 `feature/<REQ-ID>`；Task 用 detached worktree 按需隔离，同 Task 实现/测试/返修串行复用。五入口及三平台模板按当前动作路由参考，安装闭包核验与正文加载分开。未新增产品 CLI、编排脚本、业务角色、Schema 字段或 Git 包装器。

### 对象、兼容与历史保全

- 起点及当前源码 HEAD：`8aa4a05746b159e8bef45eb8f37f7cc5acda095f`，分支 `main`。受检对象是其上的未提交工作区；`tested_sha` 为 null，不把起点 Commit 或合成 fixture Commit 当成本轮已提交产品。
- 最终指令候选集合 SHA256：`cae2ad69e2ea7e4d646f43cb3445d78ce712f08a65de8ccc46aa719fa9f8eff7`，107 个清单项，算法沿用验证指南。报告自身不参与集合摘要，但参与链接/历史保全检查。
- 追加前报告与 HEAD 逐字一致：81777 字节，SHA256 `176a89d057e13339b95e9d19a814f83550590a658e9632160616ce7e58a8ecc6`；本轮仅追加此节。
- 原生模型/推理档位/工具权限与五份 UI 调用策略保持；16 个 v1 模板逐字一致，4 个 Compact 模板载荷兼容。原 SHA、报告、角色所有权、G0–G10 和 VERIFIED/DONE 条件未修改。
- 五入口正文由 58,293 字节减到 46,491 字节（约 20.25%）。阶段细节移至 Planner/Tester 工作流与 Reviewer 审核输出参考；该数值只描述文件体积，不证明实际 token、耗时或长期分支数量改善。

### 实际命令与机械结果

执行 `docs/DEVFLOW_SKILLS_VALIDATION_GUIDE.md` 的完整静态块、普通复制/负控块和新增 Git 机制块；用 Python 正则定位文档中 Bash 块，再交给 `bash -c`，没有另增产品检查器。环境实测 Python `3.14.4`、PyYAML `6.0.3`。

- 五个 `quick_validate.py` 均退出 0；13 YAML、5 TOML、16 frontmatter、89 Markdown 的解析及当前/历史相对链接、锚点通过。单角色传递闭包不依赖无关角色，发布源仍只有五个 Skill。
- 普通复制与检查器负控：12 项结果符合预期；完整安装复制 100 个文件，入口单独复制 77 个文件，重复安装拒绝及冲突零部分写入成立。重复 YAML 键、损坏 TOML、额外 Skill、坏链接/锚点、历史前缀破坏、frontmatter 与验证器失败均被拒绝。预期非零退出作为负向证据保留，未隐藏失败。
- 隔离 Git：11 项 PASS，103 条 Git 命令，9 次预期非零。实际覆盖唯一 `feature/REQ-WORKTREE-CHECK`、3 个并行 detached Task、同工作区实现/测试/返修、快进与普通合并、真实冲突交回作者、新 SHA、续作分支复用、原脏文件保护、脏/锁定 worktree 拒绝清理、历史候选可达、squash 后 bundle 空对象库恢复。
- Git fixture 已清理；原始命令/结果与 bundle 保留在本次临时诊断目录，复现流程完整保存在指南。机制执行时指南摘要为 `8e15dcdf151b7a09081cd75a6608ed44ff03c4cb8c715963bc4a283138073dfa`；后续仅补了 Planner/作者模拟边界说明，执行代码未变，未因说明改动重跑。
- `git diff --check`、`git diff --cached --check` 通过；无源码仓库提交、推送、真实业务仓库或已安装 Skill 写入。

### 独立行为评测与实际读取

独立执行者仅接收原始场景、Skill 路径与合成材料，不接收预期答案。以下为本轮实际读取/操作的归纳，不是把路由文字命中当作行为通过。

| 场景 | 实际观察 |
|---|---|
| 完整安装后的入口路由 | 实际读取入口与 Planner 两份 SKILL；bootstrap 及其他角色仅核验可读性，未读安装正文或业务流程。路由到 Planner，按评测边界停在首次业务动作前，无实际原生派发。 |
| CODE_REVIEW 首轮 | 实际只选择代码模式并审核真实 fixture SHA；但 Git 策略被整篇读取，暴露可进一步收紧的加载范围。没有读取其他审核模式或安装恢复。 |
| CODE_REVIEW 修正后独立复验 | 入口/代码模式增加精确章节路由后，新执行者仅读 Git 的“候选身份核验”正文（29–32 行）及标题/匹配行，不读工作区创建/回收流程。相关 Gate、生命周期、Brownfield、Python 与原模板按当前动作读取；实际检查精确对象行为并返回合成 APPROVE，不代替 Tester PASS。 |
| 测试计划 | 实际读取 Tester 入口、testing-workflow 的第 2 节、测试策略、相关契约与 Compact 计划模板；不读代码交付、执行/失败流程、平台安装。写出 7 个用例且全部 not_run，没有运行受测函数或伪造测试 PASS。 |
| 单角色缺入口 | 固定同源本地合成候选下，现有 Reviewer 8 文件匹配，实际排他补齐入口 77 文件；结束后 85 文件逐字匹配，仅存在入口和 Reviewer 两个 Skill。未写原生配置或无关角色。 |
| 单角色定制冲突 | Reviewer 的 code-review 参考与候选不一致，目标保持零写入；主维护者另核对原 8 文件的内容摘要及 mtime 均未改变。 |

原始业务 fixture 的 base/head 为 `6a24f1335ce177d0847b02482477bcc189fa6581..1f52013c0f649351abc9e8120680c9375f5dafee`；内容是整数乘法增加负数量 ValueError。审批、宿主身份与安装已加载前提明确为合成输入。安装 fixture 的固定候选为 `c0634c6980a9d8f2f4ca99dacec2b4470a4264ca`，只在临时来源仓库生成，不属于源码提交。

独立只读变更审核发现 1 项 P2：G5 前测试代码动作未路由到迁出的实现原则。已增加“测试实现原则”独立锚点和动作链接；同一审核者复核精确 Delta 后关闭。未恢复首次全量读取。后续源码修订仅涉及该测试代码路由、Reviewer 候选身份路由及 Implementer 原报告 SHA 说明；未改变先前入口/安装守卫/测试计划场景的规则，未以重跑无关场景制造新的通过记录。

读取轨迹和原始完整输出保留于本次临时诊断资料。以下原件摘要用于区分各轮结果；此表不是运行期报告 Schema 或永久状态矩阵：

| 评测原件 | SHA256 |
|---|---|
| `entry-result.md` | `e4e7fa4f07fa80cd2762ef778a7e141bef2fbcd8d9d5c9280d6bf1f2426ddc7f` |
| `reviewer-result.md` | `6ca69bc6e25db732c5aa595e431f7514305e18398d9cb5298a032957b11246bf` |
| `reviewer-current-result.md` | `3fc80bdd3ccb958e086db7993e6c975924d7f2eec6f6f8ad1cbab43224b7912a` |
| `tester-result.md` | `66a82e328f52058a4fba9e470a1b6cee70253601f9d958c2fc68598d3913f1d7` |
| `bootstrap-result.md` | `074ab73e6f9c863480a2ce269a916638f8316378e6ef585ed3093df4830ef8a0` |

复验方式：按工作流评测 WF-45–50 与触发评测 TRIGGER-19–23 的原始输入准备隔离材料，只将请求/身份/允许范围交给执行者；事后核对实际工具读取与文件结果，不把判定段提前给执行者。工作区换基线/回收机制按指南末段执行。原始诊断目录不是正式业务历史的唯一来源，也不能用这些 fixture 身份通过真实 Gate。
