...
G5 后发生生产或测试代码变化时，受影响 Task 经 `REWORK -> CODE_REVIEW` 重新绑定候选并完成集成验证；期间不能继续作为有效 `VERIFIED`/`DONE` 解锁依赖。已启动的下游按实际影响暂停并补证，无关 Task 不重置。详见[测试代码交付](../../devflow-tester/references/test-code-delivery.md)。
