...
复用 [Implementation Report](../../_devflow_shared/templates/implementation-report.yaml) 的全部原字段、`schema_version: 1` 和唯一 `IMPL-NNN`；`task_id` 为原 Task，已归因测试问题时 `defect_id` 为原 Defect。
